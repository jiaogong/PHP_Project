...
/**
 * global function
 * $Id: global.func.php 2920 2016-06-06 02:30:23Z david $
...
function replaceNewsURL($str) {
    if (RWURL === 1) {
        ...
        } elseif (strpos($str, '&ids=65') !== false) {
            $str = preg_replace('/\/?article\.php\?action=CarReview&id=[\d]+&ids=\d+&page=([\d]+)/im', '/lvxing/p\1/', $str);
        } else {
...
function replaceWapNewsUrl($str) {
    global $local_host;
    if (RWURL) {
        $str = str_replace($local_host, '/', $str);
        $str = str_replace('"waparticle.php', '"/waparticle.php', $str);
        $str = preg_replace('/\/waparticle\.php\?id=(\d+)/im', 'http://m.' . DOMAIN . '/\1.html', $str);
        #$str = preg_replace('/[^\?]+waparticle\.php\?id=(\d+)/im', 'http://m.' . DOMAIN . '/\1.html', $str);
    }
    return $str;
}
 ...
 * 
 * @global global string $local_host 定义在include/config.php中站点链接
...
function replaceNewsChannel($str) {
    ...
    if (RWURL === 1) {
        ...
        $str = str_replace('<a href="/garage.php">车型</a>', '<a href="' . $local_host . 'car/">车型</a>', $str);
        $str = str_replace('<a href="/search.php?action=index">选车</a>', '<a href="' . $local_host . 'xuanche/">选车</a>', $str);
        $str = str_replace('<a href="/pic.html" target="_blank">图片</a>', '<a href="' . $local_host . 'photo/">图片</a>', $str);
 ...
 * 需要根据 RWURL常量判断
 * 
 * @global global string $local_host 站点链接
 * @param string $str 要处理的原始字符串
 * @return string 替换完成的字符串
 */
 ...
 * @return string 替换完成的字符串
 */
function replaceReviewUrl($str) {
    if (RWURL === 1) {
        #$str = preg_replace('%/article\.php\?action=review&type=(\d+)&article_id=(\d+)&page=(\d+)%im', '/club/\2-\1-\3.html', $str);
        $str = preg_replace('%/review\.php\?action=list&type=(\d+)&article_id=(\d+)&type_id=(\d+)&page=(\d+)%im', '/club/\2-\1-\3-\4.html', $str);
        $str = preg_replace('%/review\.php\?action=list&type=(\d+)&article_id=(\d+)&type_id=(\d+)%im', '/club/\2-\1-\3.html', $str);
    }
    return $str;
}
...
function replaceImageUrl($html){
    ...
    if($attach_server){
        $html = str_replace($local_host . 'attach/images/', $attach_server[0] . '/', $html);
        $html = str_replace('/attach/images/', $attach_server[0] . '/', $html);
    }
    return $html;
}
 ...
 */
function cookie($name, $value = '', $ltime = 3600) {
    global $timestamp;
    if ($value) {
...
function escape($str) {
    ...
    foreach ($ar as $k => $v) {
        if (ord($v[0]) < 223)
            ...
            $ar[$k] = "%u" . strtoupper(bin2hex(mb_convert_encoding($str[$i], "UCS-2", "GBK")));
    }
    return join("", $ar);
}
 ...
 * @return array 指定的数组
 */
function receiveArray($gp, $incharset = "gbk") {
    $receiver = array();
    if ($incharset == "utf-8") {
        foreach ($gp as $value) {
            $receive = $_GET[$value] ? iconv("UTF-8", "GBK", $_GET[$value]) : ($_POST[$value] ? iconv("UTF-8", "GBK", $_POST[$value]) : '');
            if ($receive)
                $receiver[$value] = $receive;
        }
    }else {
        foreach ($gp as $value) {
            $receive = $_GET[$value] ? $_GET[$value] : ($_POST[$value] ? $_POST[$value] : '');
            if ($receive)
                $receiver[$value] = $receive;
        }
    }
...
function message($url, $str, $time = 3) {
    ...
    $message = '
      <div class="wrap">
        <div class="logo1"><a href="#"><img src="' . $admin_path . 'images/logo.png" /></a></div>
        <div class="logout">
          <div class="logout_con">
            <h2>' . $str . '</h2>
            <h1>如果你的浏览器没有自动跳转，请<a href="' . $url . '">点击此链接</a>。</h1>
          </div>
        </div>
      </div>
...
/**
 * 针对搜索URL的处理方法
 * 过滤掉URL中指定的参数后，返回重组的URL字符串
 * 
 * @param string $qas 原URL字符串
 * @param string $strip 要过滤掉的URL参数字符串
 * @param string $i 
 * @return string 重组后的URL字符串
 */
function qaslastStrip($qas, $strip, $i) {
    $str = "";
    $tmp = explode('&', $qas);
    foreach ($tmp as $k => $v) {
        if (strpos($v, $i) !== false)
 ...
 * 
 * @param string $qas 原URL字符串
 * @param array $strip 要过滤掉的URL参数数组
 * @return string 重组后的URL字符串
 */
function qasStripField($qas, $strip = array()) {
    $str = "";
    $tmp = explode('&', $qas);
    foreach ($tmp as $k => $v) {
        $t = explode('=', $v);
        if (in_array($t[0], $strip))
            continue;
        if ($str) {
            $str .= "&" . $v;
        } else {
...
/**
 * 处理搜索页面中出现的QueryString
 * 
 * @param string $url 搜索页面中的URL
 * @param string $strip 需要过滤掉的字符串
 * @return array 返回处理后的名称=>值的数组
 */
function parseQas($url, $strip = '') {
    $querystring = $url;
    $qa = array();
...
function setErrorHandler($errno, $errstr, $errfile, $errline) {
    ...
    switch ($errno) {
        case E_USER_ERROR:
            echo "<b>ERROR</b> [$errline] $errstr<br />\n";
        ...
        case E_USER_WARNING:
            echo "<b>WARNING</b> [$errline] $errstr<br />\n";
            break;

        case E_USER_NOTICE:
            echo "<b>NOTICE</b> [$errline] $errstr<br />\n";
            break;

        default:
            echo "Unknown error type: [$errline] $errstr<br />\n";
            break;
    }
    return true;
}
...
function getDeliveryPrice($bingoPrice) {
    if ($bingoPrice == 0)
        $deliveryPrice = 0;
    else
        $deliveryPrice = round($bingoPrice / 11.7 + $bingoPrice, 2) * 10000 + 950 + 373;
