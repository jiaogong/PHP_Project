(function(f){
    ...
    d.scriptPath=function(){
        for(var a=document.getElementsByTagName("script"),b=0,c=a.length;b<c;b++)if(a[b].src&&a[b].src.match(/kindeditor[\w\-\.]*\.js/))return a[b].src.substring(0,a[b].src.lastIndexOf("/")+1);return""
        }();
    d.browser=function(){
    ...
    statusbarHeight:11,
    items:["source","|","fullscreen","undo","preview","redo","print","cut","copy","paste","plainpaste",
    "wordpaste","|","justifyleft","justifycenter","justifyright","justifyfull","insertorderedlist","insertunorderedlist","indent","outdent","subscript","superscript","|","selectall","-","title","fontname","fontsize","|","textcolor","bgcolor","bold","italic","underline","strikethrough","removeformat","|","image","flash","media","advtable","hr","emoticons","link","unlink","pagebreak","|","about"],
...
d.event={
    ...
    remove:function(a,b,c,e){
        if(a.removeEventListener)a.removeEventListener(b,c,false);else a.detachEvent&&a.detachEvent("on"+b,c);
        if(e!==f){
            e=d.g[e].eventStack;
...
if(c.addEventListener)this.add(c,"DOMContentLoaded",h,e);
    ...
    this.add(c,"readystatechange",function(){
...
d.selection=function(a){
    ...
    this.init=function(){
        var c=b.getSelection?b.getSelection():a.selection,e;
        try{
        ...
        if(d.browser.IE)if(e.item){
            ...
            c=function(l){
                var m=e.duplicate();
                m.collapse(l);
                var p=m.parentElement(),o=p.childNodes;
...
function(c){
    if(!(d.browser.GECKO&&d.browser.VERSION<3)){
        ...
        if(d.browser.IE){
            var e=function(j){
                var k=d.util.createRange(a),l=j?c.startNode:c.endNode;
                if(l.nodeType==1){
...
this.getNodeList=function(){
    var b=this,c=this.getParentElement(),e=[],g=false;
    if(c==b.startNode)g=true;
    g&&e.push(c);
    d.eachNode(c,function(h){
    ...
    var e=function(g,h,i,j){
        if(d.browser.IE){
            var k=function(m,p,o){
                var q=d.util.createRange(a),r=d.util.getNodeType(m);
                if(r==1){
...
this.extractContents=function(b){
    b=b===f?true:b;
    var c=this,e=this.startNode,g=this.startPos,h=this.endNode,i=this.endPos,j=function(r,u,n){
        var v=r.nodeValue.length,t=r.cloneNode(true).splitText(u);
        t.splitText(n-u);
        if(b){
            var x=r;
    ...
    k=d.util.arrayToHash(d.setting.noEndTags),l=false,m=false,p=function(r,u){
        if(d.util.getNodeType(r)!=1)return true;
        for(var n=r.firstChild;n;){
            if(n==e)l=true;
            if(n==h)m=true;
            var v=n.nextSibling,t=n.nodeType;
            if(t==1){
                t=new d.range(a);
                t.selectNode(n);
...
return true
},o=this.getParentElement(),q=o.cloneNode(false);
p(o,q);
return q
};
...
d.cmd=function(a){
    ...
    this.mergeAttributes=function(b,c){
        for(var e=0,g=c.length;e<g;e++)d.each(c[e],function(h,i){
            if(h.charAt(0)=="."){
...
this.wrapTextNode=function(b,c,e,g,h){
    var i=b.nodeValue.length,j=c==0&&e==i,k=new d.range(this.doc);
...
this.wrap=function(b,c){
    ...
    var h=this.keRange,i=h.startNode,j=h.startPos,k=h.endNode,l=h.endPos,m=h.getParentElement();
    if(!d.util.inMarquee(m)){
        var p=false;
        d.eachNode(m,function(o){
            if(o==i)p=true;
...
function(b,c){
    for(var e=null;c;){
        c=c.parentNode;
        if(d.util.inArray(c.tagName.toLowerCase(),b))e=c;else break
    }
    return e
    };
...
this.remove=function(b){
    var c=this.keRange,e=c.startNode,g=c.startPos,h=c.endNode,i=c.endPos;
    this.keSel.focus();
    if(!d.util.inMarquee(c.getParentElement())){
        ...
        if(!(j&&
            ...
            var l=this.getTopParent(k,e),m=this.getTopParent(k,h);
            if(l){
...
d.format={
    getUrl:function(a,b,c,e){
        ...
        g=function(i){
            ...
            for(var j=0,k=i.length;j<k;j++){
                var l=i[j];
                if(l=="..")paths.length>0&&paths.pop();else l!==""&&l!="."&&paths.push(l)
                    }
                    return"/"+paths.join("/")
            };

        if(a.match(/^\//))a=c+g(a.substr(1));else a.match(/^\w+:\/\//)||(a=c+g(e+"/"+a));
        if(b=="relative"){
            var h=function(i,j){
    ...
    getHtml:function(a,b,c){
        var e=b?true:false;
        a=a.replace(/(<pre[^>]*>)([\s\S]*?)(<\/pre>)/ig,function(m,p,o,q){
            return p+o.replace(/<br[^>]*>/ig,"\n")+q
            });
        var g={},h=["xx-small","x-small","small","medium","large","x-large","xx-large"];
        e&&d.each(b,function(m,p){
        ...
        a=a.replace(/((?:\r\n|\n|\r)*)<(\/)?([\w-:]+)((?:\s+|(?:\s+[\w-:]+)|(?:\s+[\w-:]+=[^\s"'<>]+)|(?:\s+[\w-:]+="[^"]*")|(?:\s+[\w-:]+='[^']*'))*)(\/)?>((?:\r\n|\n|\r)*)/g,function(m,p,o,q,r,u,n){
            ...
            if(v==="font"){
                var t={},x="";
                q=q.replace(/\s*([\w-:]+)=([^\s"'<>]+|"[^"]*"|'[^']*')/g,function(w,y,s){
                    y=y.toLowerCase();
                    s=s||"";
                    s=s.replace(/^["']|["']$/g,"");
                    if(y==="color"){
            ...
            q=q.replace(/\s*([\w-:]+)=([^\s"'<>]+|"[^"]*"|'[^']*')/g,function(w,y,s){
                ...
                if(s==="")s='""';
                    ...
                    if(w==="style"){
                        ...
                        s=s.replace(/\s*([^\s]+?)\s*:(.*?)(;|$)/g,function(z,A,B){
                            z=A.toLowerCase();
            ...
            q=q.replace(/\s+(checked|selected|disabled|readonly)(\s+|$)/ig,function(w,y){
                var s=y.toLowerCase();
    ...
    if(!d.browser.IE){
        ...
        var k=d.setting.inlineTags.join("|"),l=function(m){
        var p=m.replace(new RegExp("<("+k+")[^>]*><\\/("+k+")>","ig"),function(o,q,r){
            return q==r?"":o
            });
        if(m!==p)p=l(p);
        return p
...
d.addClass=function(a,b){
    if(typeof a=="object"){
        ...
        if(c){
            ...
            }else if(typeof a=="string")a=/\s+class\s*=/.test(a)?a.replace(/(\s+class=["']?)([^"']*)(["']?[\s>])/,function(e,g,h,i){
...
d.getComputedStyle=function(a,b){
    var c=a.ownerDocument,e=c.parentWindow||c.defaultView;
    c=d.util.getJsKey(b);
    var g="";
...
d.getCommonAncestor=function(a,b){
    function c(i){
        ...
        var e=a.range,g=a.keRange,h=g.startNode;
    g=g.endNode;
    if(d.util.inArray(b,
...
d.queryCommandValue=function(a,b){
    ...
    if(b==="fontname"){
        ...
        e=c();
        if(e===""){
            var g=new d.selection(a),h=d.getCommonAncestor(g,"h1");
...
inArray:function(a,b){
    for(var c=0;c<b.length;c++)if(a==b[c])return true;return false
    },
trim:function(a){
...
setOpacity:function(a,b){
    if(typeof a.style.opacity=="undefined")a.style.filter=b==100?"":"alpha(opacity="+b+")";else a.style.opacity=b==100?"":b/100
        },
...
rgbToHex:function(a){
    function b(c){
        ...
        return a.replace(/rgb\s*?\(\s*?(\d+)\s*?,\s*?(\d+)\s*?,\s*?(\d+)\s*?\)/ig,function(c,e,g,h){
        return"#"+b(e)+b(g)+b(h)
        })
    },
parseJson:function(a){
    ...
    b=/[\u0000\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g;
    b.lastIndex=0;
    if(b.test(a))a=a.replace(b,function(c){
...
getNodeStartRange:function(a,b){
    var c=d.util.createRange(a),e=b.nodeType;
    if(e==1){
...
pluginLang:function(a,b){
    d.each(d.lang.plugins[a],function(c,e){
        var g=d.$("lang."+c,b);
        if(g){
...
drag:function(a,b,
    ...
    b.onmousedown=function(h){
        function i(s){
            if(x){
                var z=d.util.getCoords(s),A=d.util.getScrollPos();
    ...
    var l=this;
h=h||window.event;
var m=d.util.getCoords(h),p=parseInt(c.style.top),o=parseInt(c.style.left),q=c.style.width,r=c.style.height;
...
var v=m.y,t=m.x,x=true,w=d.util.getElementPos(g.iframe),y=function(){
    return false
    };
...
setDefaultPlugin:function(){
    for(var a=["selectall","justifyleft","justifycenter","justifyright","justifyfull","insertorderedlist","insertunorderedlist","indent","outdent",
        ...
        if(e in b)g.init=function(h){
            return function(i){
                d.event.ctrl(d.g[i].iframeDoc,b[h],function(){
                    d.plugin[h].click(i);
                    d.util.focus(i)
                    },i)
                }
...
getFullHtml:function(a){
    ...
    for(var c=0,e=a.length;c<e;c++)if(a[c]!=="")b+='<link href="'+a[c]+'" rel="stylesheet" type="text/css" />';b+="</head>";
...
toData:function(a,b){
    var c=d.g[a],e=this.execGetHtmlHooks(a,b);
...
pasteHtml:function(a,b,c){
    a=d.g[a];
    if(c)b='<img id="__ke_temp_tag__" width="0" height="0" />'+b;else b+='<img id="__ke_temp_tag__" width="0" height="0" />';
    if(d.browser.IE)if(a.range.item)a.range.item(0).outerHTML=b;else a.range.pasteHTML(b);
    else{
...
addTabEvent:function(a){
    d.event.add(d.g[a].iframeDoc,"keydown",function(b){
        if(b.keyCode==9){
...
addContextmenuEvent:function(a){
    var b=d.g[a];
    b.contextmenuItems.length!=0&&d.event.add(b.iframeDoc,"contextmenu",function(c){
        d.hideMenu(a);
        d.util.setSelection(a);
        ...
        h=0;
        for(i=g.length;h<i;h++)if(j=g[h])if(j==="-")h<i-1&&l.addSeparator();else l.add(j.text,function(m){
            return function(){
                m.click(a,l)
                }
            }(j),j.options);l.show();
...
addNewlineEvent:function(a){
    ...
    "br"||d.browser.GECKO&&d.browser.VERSION<3&&b.newlineTag.toLowerCase()!="p"||d.browser.OPERA||d.event.add(b.iframeDoc,"keydown",function(c){
...
d.colorpicker=function(a){
    var b,c=a.x||0,e=a.y||0,g=a.z||0,h=a.colors||d.setting.colorTable,i=a.doc||document,j=a.onclick,k=(a.selectedColor||"").toLowerCase();
    b=d.$$("div");
    ...
    this.getElement=function(){
        ...
        m.cellSpacing=0;
        m.border=0;
        var p=m.insertRow(0),o=p.insertCell(0);
...
this.create=function(){
    ...
    d.event.bind(b,"click",function(){});
    d.event.bind(b,"mousedown",function(){});
...
d.menu=function(a){
    ...
    if(a.width)e.style.width=/^\d+$/.test(c)?
        c+"px":c;
    d.event.bind(e,"click",function(){},a.id);
    d.event.bind(e,"mousedown",function(){},a.id);
    this.div=e
    }).call(this);
this.add=function(c,e,g){
    ...
    g.appendChild(m);
    g.appendChild(p);
    if(j)d.util.innerHtml(l,'<span class="ke-common-icon ke-common-icon-url ke-icon-checked"></span>');else i&&d.util.innerHtml(l,i);
...
d.dialog=function(a){
    function b(){
        ...
        function c(){
        var h=this.width+this.widthMargin,i=this.height+this.heightMargin,j=a.id,k=d.g[j],l=0,m=0;
...
function(){
    ...
    var h=a.id,i=d.g[h].dialogStack;
...
this.show=function(){
    ...
    var h=this,i=a.id,j=d.$$("div");
    j.className="ke-dialog";
    d.event.bind(j,
        "click",function(){},i);
    d.event.bind(j,"mousedown",function(){},i);
...
able:function(a,b){
    var c=this;
    d.each(d.g[a].toolbarIcon,function(e,g){
...
disable:function(a,b){
    d.each(d.g[a].toolbarIcon,function(c,e){
...
create:function(a){
    ...
    d.g[a].toolbarIcon=[];
    var c=d.util.createTable(),e=c.table;
    ...
    c=c.cell;
    var g=d.g[a].items.length,h=0,i;
...
d.history={
    ...
    add:function(a,b){
        var c=d.g[a],e=d.util.getSrcData(a);
    ...
    undo:function(a){
        ...
        if(b.undoStack.length!=0){
            ...
            e=d.util.toData(a,
                e);
            if(b.wyswygMode)d.util.innerHtml(b.iframeDoc.body,d.util.execSetHtmlHooks(a,e));else b.newTextarea.value=e
                }
            },
redo:function(a){
    var b=d.g[a];
    if(b.redoStack.length!=0){
        ...
        c=b.redoStack.pop();
        c=d.util.toData(a,c);
        if(b.wyswygMode)d.util.innerHtml(b.iframeDoc.body,d.util.execSetHtmlHooks(a,c));else b.newTextarea.value=c
            }
        }
};
...
d.readonly=function(a,b){
    b=b==f?true:b;
    var c=d.g[a];
    if(d.browser.IE)c.iframeDoc.body.contentEditable=b?"false":"true";else c.iframeDoc.designMode=
        b?"off":"on"
        };
...
d.focus=function(a,b){
    ...
    if(d.g[a].container){
        ...
        if(b==="end"){
            d.util.setSelection(a);
            if(d.g[a].sel){
                var c=d.g[a].keSel,e=d.g[a].keRange;
...
d.create=function(a,b){
    ...
    if(!(b==0&&d.g[a].container)){
        var j=d.g[a].width||i.style.width||i.offsetWidth+"px",k=d.g[a].height||i.style.height||i.offsetHeight+"px",l=d.util.createTable(),m=l.table;
        ...
        var n=r.insertRow(0),v=n.insertCell(0);
        ...
        d.event.bind(t,"click",function(){},a);
        d.event.bind(t,"mousedown",function(){},a);
        document.body.appendChild(u);
        document.body.appendChild(t);
        d.util.setDefaultPlugin(a);
        var x=p.contentWindow,w=d.util.getIframeDoc(p);
        ...
        if(!d.g[a].wyswygMode){
            ...
            d.browser.WEBKIT&&d.event.add(w,"click",function(s){
            d.util.selectImageWebkit(a,s,true)
            },a);
        d.browser.IE&&
        d.event.add(w,"keydown",function(s){
            if(s.keyCode==8){
                s=d.g[a].range;
                if(s.item){
                    s=s.item(0);
...
d.g[a].height=
k;
d.util.drag(a,n,m,function(s,z,A,B,C,D){
    if(d.g[a].resizeMode==2)d.util.resize(a,A+D+"px",B+C+"px",true);else d.g[a].resizeMode==1&&d.util.resize(a,A+"px",B+C+"px",true,false)
        });
d.util.drag(a,v,m,function(s,z,A,B,C){
    d.g[a].resizeMode>0&&d.util.resize(a,A+"px",B+C+"px",true,false)
    });
d.each(d.plugin,function(s,z){
    z.init&&z.init(a)
    });
d.g[a].getHtmlHooks.push(function(s){
    return s.replace(/(<[^>]*)kesrc="([^"]+)"([^>]*>)/ig,function(z,A,B){
...
d.g[a].setHtmlHooks.push(function(s){
    return s.replace(/(<[^>]*)(href|src)="([^"]+)"([^>]*>)/ig,function(z,A,B,C,D){
...
d.onchange=function(a,b){
    ...
    d.event.add(e.iframeDoc,"mouseup",function(){
        window.setTimeout(function(){
            b(a)
            },0)
        },a)
...
(function(f,d){
    f.plugin.pagebreak=
    {
        click:function(a){
            f.util.selection(a);
            f.util.insertHtml(a,"<hr style='page-break-after:always;border: 1px dotted #AAAAAA;font-size: 0;height: 2px;margin:10px 0px 10px 0px;'/>");
            f.util.focus(a)
            }
        };
...
f.plugin.undo={
    init:function(a){
        f.event.ctrl(f.g[a].iframeDoc,"Z",function(){
            f.plugin.undo.click(a);
            f.util.focus(a)
            },a);
        f.event.ctrl(f.g[a].newTextarea,"Z",function(){
            f.plugin.undo.click(a);
            f.util.focus(a)
            },a)
        },
...
{
    init:function(a){
        f.event.ctrl(f.g[a].iframeDoc,"Y",function(){
            f.plugin.redo.click(a);
            f.util.focus(a)
            },a);
        f.event.ctrl(f.g[a].newTextarea,"Y",function(){
            f.plugin.redo.click(a);
            f.util.focus(a)
            },a)
        },
...
if(this.isSelected){
    ...
    e();
    f.create(a,1);
    var i=f.util.getScrollPos(),j=b.container;
...
f.plugin.fontname={
    click:function(a){
        ...
        f.each(b,function(g,h){
            c.add('<span class="ke-reset" style="font-family: '+g+';">'+h+"</span>",function(){
...
f.plugin.fontsize={
    click:function(a){
        ...
        for(var c=f.queryCommandValue(f.g[a].iframeDoc,"fontsize"),e=new f.menu({
            ...
            var i=b[g];
            e.add('<span class="ke-reset" style="font-size: '+i+';">'+i+"</span>",function(j){
                return function(){
...
f.plugin.title={
    click:function(a){
        ...
        f.each(b,function(h,i){
            ...
            g.add('<span class="ke-reset" style="'+j+'">'+i+"</span>",function(){
...
var e=this,g=5,h=9,i=0,j=g*h,k=Math.ceil(135/j),l=Math.floor(h/2),m=f.g[a],p=m.pluginsPath+"emoticons/";
    ...
    init:function(a){
        f.g[a].getHtmlHooks.push(function(b){
            return b.replace(/<img[^>]*class="?ke-flash"?[^>]*>/ig,
                function(c){
                    var e=c.match(/style="[^"]*;?\s*width:\s*(\d+)/i)?RegExp.$1:0,g=c.match(/style="[^"]*;?\s*height:\s*(\d+)/i)?RegExp.$1:0;
    ...
    f.g[a].setHtmlHooks.push(function(b){
        return b.replace(/<embed[^>]*type="application\/x-shockwave-flash"[^>]*>(?:<\/embed>)?/ig,
            function(c){
                ...
                var g=c.match(/\s+width="([^"]+)"/i)?RegExp.$1:0,h=c.match(/\s+height="([^"]+)"/i)?RegExp.$1:0;
...
exec:function(a){
    var b=f.util.getIframeDoc(this.dialog.iframe),c=f.$("url",b).value,e=f.$("width",b).value;
    b=f.$("height",b).value;
...
f.plugin.image={
    getSelectedNode:function(a){
        a=f.g[a];
        var b=a.keRange.startNode,c=a.keRange.endNode;
...
check:function(){
    var a=f.util.getIframeDoc(this.dialog.iframe),b=f.$("type",a).value,c=f.$("imgWidth",a).value,e=f.$("imgHeight",a).value;
    f.$("imgTitle",a);
...
exec:function(a){
    var b=f.g[a];
    f.util.select(a);
    var c=b.keRange,e=c.startNode,g=c.endNode,h=b.iframeDoc,i=f.util.getIframeDoc(this.dialog.iframe),j=f.$("hyperLink",i).value,k=f.$("linkType",i).value;
    ...
    if(l||f.util.isEmpty(a)){
        ...
        for(var m=i.length;l<m;l++)if(i[l].href.match(/\/?__ke_temp_url__$/)){
            ...
            if(k)i[l].target=k;else i[l].removeAttribute("target")
                }
            ...
            if(k)g.target=k;else g.removeAttribute("target");
            b.keSel.addRange(c)
            }
        }
    f.util.execOnchangeHandler(a);
...
click:function(a){
    var b=f.g[a],c=b.iframeDoc;
    f.util.selection(a);
    var e=b.keRange,g=e.startNode;
    e=e.endNode;
    e=g.nodeType==1&&g===e;
    var h=!e;
    e||(h=
...
f.plugin.media={
    init:function(a){
        ...
        f.g[a].getHtmlHooks.push(function(c){
            return c.replace(/<img[^>]*class="?ke-\w+"?[^>]*>/ig,function(e){
                var g=e.match(/style="[^"]*;?\s*width:\s*(\d+)/i)?RegExp.$1:0,h=e.match(/style="[^"]*;?\s*height:\s*(\d+)/i)?RegExp.$1:0;
    ...
    f.g[a].setHtmlHooks.push(function(c){
        return c.replace(/<embed[^>]*type="([^"]+)"[^>]*>(?:<\/embed>)?/ig,function(e,g){
            if(typeof b[g]=="undefined")return e;
            var h=e.match(/\s+src="([^"]+)"/i)?RegExp.$1:"";
            if(e.match(/\s+kesrc="([^"]+)"/i))h=RegExp.$1;
            var i=e.match(/\s+width="([^"]+)"/i)?RegExp.$1:0,j=e.match(/\s+height="([^"]+)"/i)?RegExp.$1:0,k=f.util.getAttrList(e);
            k.src=h;
            k.width=i;
            k.height=j;
...
exec:function(a){
    var b=f.util.getIframeDoc(this.dialog.iframe),c=f.$("url",b).value,e=f.$("width",b).value,g=f.$("height",b).value;
...
f.plugin.advtable={
    ...
    tablerowinsert:function(a,b){
        var c=this.getSelectedTable(a),e=this.getSelectedRow(a);
        ...
        for(var b=this,c="prop,colinsertleft,colinsertright,rowinsertabove,rowinsertbelow,coldelete,rowdelete,insert,delete".split(","),e=0,g=c.length;e<g;e++){
            ...
            f.g[a].contextmenuItems.push({
                ...
                return f.util.inArray(i,
                    ["tableprop","tabledelete"])?function(j){
...
exec:function(a){
    var b=f.util.getIframeDoc(this.dialog.iframe),c=f.$("mode",b),e=f.$("rows",b),g=f.$("cols",b),h=f.$("width",b),i=f.$("height",b),j=f.$("widthType",b),k=f.$("heightType",b),l=f.$("padding",b),m=f.$("spacing",
        b),p=f.$("align",b),o=f.$("border",b),q=f.$("borderColor",b),r=f.$("backgroundColor",b);
    b=e.value;
    var u=g.value,n=h.value,v=i.value;
    j=j.value;
    var t=k.value;
    k=l.value;
    var x=m.value;
    ...
    if(b==""||b==0||!b.match(/^\d*$/)){
        ...
        if(k!=="")b.cellPadding=k;else b.removeAttribute("cellPadding");
        if(x!=="")b.cellSpacing=x;else b.removeAttribute("cellSpacing");
        if(p!=="")b.align=p;else b.removeAttribute("align");
