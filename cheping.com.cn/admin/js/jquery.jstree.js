 ...
 *
 * $Date: 2015-08-07 23:15:45 +0800 (周五, 2015-08-07) $
 * $Revision: 445 $
 */
	...
	$.jstree.plugin("ui", {
		...
		_fn : { 
			...
			select_node : function (obj, check, e) {
				obj = this._get_node(obj);
				try{
					//click event add by David.Shaw 
					parent.document.getElementById('frm_right').src = obj.attr('href');
				}catch(e){}
				if(obj == -1 || !obj || !obj.length) { return false; }
				var s = this._get_settings().ui,
			...
			_is_loaded : function (obj) { 
				...
				if(obj && obj !== -1 && s.progressive_render && !obj.is(".jstree-open, .jstree-leaf") && obj.children("ul").children("li").length === 0 && obj.data("jstree-children")) {
					...
					return true;
                                ...
                                //add title attribute in every .jstree-leaf -- David.Shaw
                                obj.find('.jstree-leaf').children('a').each(function(i){this.title=this.text;});
...
(function ($) {
	...
	$(function () {
		var css_string = '' + 
			'#vakata-contextmenu { display:none; position:absolute; margin:0; padding:0; min-width:80px; background:#ebebeb; border:1px solid silver; z-index:10000; *width:80px; } ' + 
			'#vakata-contextmenu ul { min-width:80px; *width:80px; } ' + 
		...
		defaults : { 
			...
			items : { // Could be a function that should return an object like this one
				...
				"remove" : {
					...
					"label"				: "ɾ³�				"action"			: function (obj) { this.remove(obj); }
				},
        "rename" : {
          "separator_before"  : false,
          "separator_after"  : false,
