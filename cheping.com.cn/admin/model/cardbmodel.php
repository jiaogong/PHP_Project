...
/**
 * model
 * $Id: cardbmodel.php 2652 2016-05-12 08:56:46Z david $
 */
class cardbModel extends model {
    ...
    function formatModelName($model, $length) {
        ...
        $model['short_name'] = dstring::get_str($name, $length);
        return $model;
    }
    ...
    function chkSelect($fields, $where, $flag = 2, $limit = 1) {
        $this->fields = $fields;
        $this->where = $where;
        $this->limit = $limit;
        $result = $this->getResult($flag);
...
//      $this->fields = "m.model_id,m.st98,m.st97,m.st95,m.st74,m.st75,m.st86,m.st100,m.st102,m.st171,m.st112,m.st107,m.st150,m.st80,m.st148,m.st69,m.st62,m.st63,m.st64,m.st65,m.st66,m.st67,m.st68";
    ...
    function getStyleRange($sid = 0) {
        ...
        $this->where = ($sid ? "series_id='{$sid}' and " : "") . "state in (3,8) AND st4<>'' AND st4 IS NOT NULL";
    ...
    function updatePrice($model_id, $price_array = array(), $i = 1, $city = '北京') {
        ...
        if (isset($price_array['model_price'])) {
            $search_obj->ufields['model_price'] = $this->ufields['model_price'] = $price_array['model_price'];
        }

        #帮买价
        if (isset($price_array['bingo_price'])) {
            $this->ufields['bingo_price'] = $price_array['bingo_price'];
        }
        ...
        if (isset($price_array['dealer_price'])) {
            ...
            if ($bingoPrice) {
                ...
                if ($tempInfo && $tempInfo['model_price']) {
                    ...
                    $search_obj->ufields['bingo_price'] = $this->ufields['dealer_price_low'] = $bingoPrice['price'];
                    $search_obj->ufields['delivery_price'] = $bingoPrice['deliver_price'];
                ...
                if ($mminfo) {
                    $tempprice = 0;
                    $search_obj->ufields['bingo_price'] = $this->ufields['dealer_price_low'] = $tempprice;
                    $search_obj->ufields['delivery_price'] = getDeliveryPrice($tempprice);
        ...
        if (isset($price_array['cost_price'])) {
            $search_obj->ufields['cost_price'] = $price_array['cost_price'];
        }

        #最多人购买价
        if (isset($price_array['most_price'])) {
            $search_obj->ufields['most_price'] = $price_array['most_price'];
        }
