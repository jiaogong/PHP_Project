...
function model_select(obj, sid, mid) {
    ...
    <div class="user">
        <div class="nav">
            <ul id="nav">
                <li><a href="<?=$php_self?>focus" class="song">轮播图</a></li>
...
<div class="user_con">
    <div class="user_con1">
      ...
      <table cellpadding="0" cellspacing="0" class="table2" border="0">
        ...
        副标题<?=$k?>：<input type="text" name="name1_<?=$k?>" id="name1_<?=$k?>" size="40" value="<?=$v['title1']?>"><br>
        <!--精华<?=$k?>：<textarea rows="2" name="cream_<?=$k?>" id="ccream_<?=$k?>" cols="32"><?=$v['cream']?></textarea><br>-->
          导语<?=$k?>：<textarea rows="2" name="spot_<?=$k?>" id="spot_<?=$k?>" cols="32"><?=$v['spot']?></textarea><br>
