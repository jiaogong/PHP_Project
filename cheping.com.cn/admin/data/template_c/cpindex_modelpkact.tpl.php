    ...
    <div class="user_con">
        <div class="user_con1">
			...
			<table cellpadding="0" cellspacing="0" class="table2">
				<? if ($pkinfo) { ?>
					<? foreach((array)$pkinfo['value'] as $key=>$val) {?>
					<select name="brand_i[]" class="brand_id">
						<option value="">==请选择品牌==</option>
						<? foreach((array)$brand as $k=>$v) {?>
						<option <? if ($val['brand_id']==$v[brand_id]) { ?>selected="selected"<? } ?> value="<?=$v['brand_id']?>"><?=$v['letter']?> <?=$v['brand_name']?></option>
						<?}?>
					</select>&nbsp;&nbsp;&nbsp;&nbsp;
					<select name="factory_i[]" class="factory_id">
						<option value="">==请选择厂商==</option>
						<option value="<?=$val['factory_id']?>" >==请选择厂商==</option>
					</select>&nbsp;&nbsp;&nbsp;&nbsp;
					<select name="series_i[]" class="series_id">
						<option value="">==请选择车系==</option>
						<option value="<?=$val['series_i']?>" >==请选择车系==</option>
					</select>&nbsp;&nbsp;&nbsp;&nbsp;
					<select name="model_i[]" class="model_id">
						<option value="">==请选择车款==</option>
						<option value="<?=$val['series_i']?>" >==请选择车款==</option>
					</select>
					<?}?>
				<? } else { ?>
				<? for($i=0; $i<5; $i++) { ?>
