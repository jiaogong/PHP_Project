<? if(!defined('SITE_ROOT')) exit('Access Denied');?>
<? include $this->gettpl('header');?> 
<div class="user_wrap" style="width:800px">
  <div class="module_con" style="width:800px">
    <div class="module_con1" style="width:800px">
      <table cellpadding="0" cellspacing="0" class="table2">
        
        <!--tr><td colspan="5" class="td_left">
        <form action="<?=$php_self?>&father_id=<?=$fatherId?>" method="post">
        品牌
        <select name="brand_id" id="brand_id">
          <option value="">请选择品牌</option>
          <? foreach((array)$brand_list as $bk=>$bv) {?>
          <option value="<?=$bv['brand_id']?>"><?=$bv['brand_name']?></option>
          <?}?>
        </select>
        厂商名称
        <input type="text" size="20" name="keyword" id="keyword" value="">
        <input type="submit" value="  搜 索  ">
        </form>
        </td></tr-->
        
        <tr align="right"  class='th'> 
          <th height="20" nowrap>编号</th>
          <th width="30%" align="left">厂商名称</th>
          <th nowrap>属性</th>
          <th nowrap>品牌</th>
          <th nowrap>操作</th>
        </tr>
        
        <? foreach((array)$list as $k=>$v) {?>
        <tr align="right">
          <td height="20" nowrap align="center"><?=$v[factory_id]?></td>
          <td>
          <a href="<?=$php_self?>edit&factory_id=<?=$v[factory_id]?>&fatherId=<?=$v[brand_id]?>">
          <?=$v[factory_name]?>
          </a>
          </td>
          <td><? echo getCardbState($v[state]); ?></td>
          <td><?=$v[brand_name]?>(<? echo getCardbState($v[bs]); ?>)</td>
          <td nowrap align="left">
          <a href="<?=$php_self?>edit&factory_id=<?=$v[factory_id]?>&fatherId=<?=$v[brand_id]?>">
          <img src="<?=$admin_path?>images/rewrite.gif" width="12" height="12" />修改</a>&nbsp;
          <a href="#here" class="click_pop_dialog" mt='1' icon='warnning' tourl="<?=$php_self?>del&factory_id=<?=$v[factory_id]?>&fatherId=<?=$v[brand_id]?>">
          <img src="<?=$admin_path?>images/del1.gif" width="9" height="9" />删除</a>
          </td>
        </tr>
        <?}?>
        
        <tr><td colspan="5" align="center" class="page_bar_css">
        <?=$page_bar?>
        </td></tr>

      </table>
    </div>
    
  </div>
</div>

<script type="text/javascript">
$().ready(function(){
  $('.click_pop_dialog').live('click', function(){
    pop_window($(this), {message:'您确定要删除该厂商吗？', pos:[200,150]});
  });
  
  <? if ($keyword) { ?>
  $('#keyword').val("<?=$keyword?>");
  <? } ?>
  
  <? if ($brand_id) { ?>
  $('#brand_id option[value="<?=$brand_id?>"]').attr({selected:true});
  <? } ?>
});
</script>
<? include $this->gettpl('footer');?> 
