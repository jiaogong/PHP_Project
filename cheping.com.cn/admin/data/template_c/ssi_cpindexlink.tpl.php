...
<div class="footer" id="wei">
    <div id="cen_right_top">
        ...
        <div id="links-content-div" style="display:block">
            ...
            <? if ($v[pagetype]==1 && $v[type]==1) { ?>
            <a href="<?=$v[link]?>" title="<?=$v[memo]?>" displaying="ok" target="view_window" class="youlian" <? if ($v[nofollow]==1) { ?> rel="nofollow" <? } ?>><?=$v[name]?></a>
            <? } ?>
            <?}?>
        </div>
        <div id="cooperative-content-div">
            <? foreach((array)$linklist as $k=>$v) {?>
            <? if ($v[pagetype]==1 && $v[type]==2) { ?>
            <a href="<?=$v[link]?>" title="<?=$v[memo]?>" displaying="ok" target="view_window"  class="hezuo" <? if ($v[nofollow]==1) { ?> rel="nofollow" <? } ?>><?=$v[name]?></a>
            <? } ?>
            <?}?>
        </div>
...
<div class="banquan">
    <div class="yllx">友情链接请联系: QQ 171479684</div>
...
<div class="denglu" style="display:none;">
    <div class="theme-popover">
        ...
        <div class="fanweik">
            ...
            <div class="jizhu"><input name="type" id="type" type="checkbox" value=""/>&nbsp;下次自动登录</div>
