...
<div class="user">
    <div class="navs">
        <ul class="nav">
            ...
            <li><a href="<?=$php_self?>friends&a=0"<? if ($a==0) { ?>class="song"<? } ?> >首页列表页</a></li>
            <? foreach((array)$category_id as $k=>$v) {?>
            <li><a href="<?=$php_self?>friends&a=<?=$v[id]?>" <? if ($v[id]==$a) { ?>class="song"<? } ?> ><?=$v[category_name]?>列表页</a></li>
            <?}?>
