            ...
            <form action="" id="form_do" method="post" enctype="multipart/form-data">
                ...
                <table border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tbody id="tobdy">
                        ...
                        <tr>
                            <td><input type='checkbox' <? if ($v[show_enable]==1) { ?>checked<? } ?> value="<?=$v[id]?>" id="<?=$v[id]?>" name="id[]" size="4" ></td>
                            <!--                                <td>
                                                                <select style="width:80px;" name="channel_index[]">
                                                                    <option value ="no"<? if ($v[channel_index]=='no') { ?>selected="selected"<? } ?>>-选择频道-</option>
                                                                    <option value ="7"<? if ($v[channel_index]==7) { ?>selected="selected"<? } ?>>资讯</option>
                                                                    <option value ="8" <? if ($v[channel_index]==8) { ?>selected="selected"<? } ?>>评测</option>
                                                                    <option value ="9" <? if ($v[channel_index]==9) { ?>selected="selected"<? } ?>>视频</option>
                                                                    <option value ="10" <? if ($v[channel_index]==10) { ?>selected="selected"<? } ?>>文化</option>
                                                                </select>
                                                            </td>-->
                            ...
                            <td>
                                <? if ($v[created]) { ?><? echo date('Y-m-d',$v[created]) ?><? } ?>
                            </td>
                            <!--                                 <td>
                                                                <? if ($v[updated]) { ?><? echo date('Y-m-d',$v[updated]) ?><? } ?>
                                                            </td>-->
                ...
                $(".but_check").live('click',function(){
                   ...
                   $.get("<?=$php_self?>ajaxarticle",{"id":id},function(msg){
                       if(msg==-1){
                        ...
                        $('.all').click(function(){
                            $('#form_do').attr('action','index.php?action=bannerPic-make');
                            return true;
                        })
