...
/**
 * watermark class
 * $Id: imagemark.class.php 2915 2016-06-03 11:31:33Z david $
...
class imagemark {
     ...
     * 图片水印(gd lib)
     * 
     * @param resource $source_image 水印文件
     * @param Imagick $water
     * @param mixed $postion
     * @param mixed $border
     * @param resource $watermark
     * @param int $watermark_location(0为左上角,1为右上角,2为左下角,3为右下角,4为居中,5上中,6左中,7右中,8下中)
     * @param int $wm_w
     * @param int $wm_h
     * @return file
     */
    ...
    /**
     * 为指定的图片添加水印，水印是图片或文字，需要在$water中指定
     * 
     * $water参数数组格式如下：
     * <p>
     * array('type' => 'file' // text or file, 'file' => '' // watermark file path ,'text' => 'watermark' //watermark string ,'fontsize' => 15,'fontcolor' => '#FF0000','alpha' => 0.9)
     * </p>
     * @param string    $source_image   原图文件地址
     * @param array     $water          水印文件地址 
     * @param int       $postion        水印位置(0 左上,1右上，2左下，3右下,4正中,5上中,6左中,7右中,8下中)
     * @param int       $border         边距像素数
     * @param int       $lib            指定使用的图形库，0:GDlib, 1:ImageMagick
     * @return file     水印添加失败返回false, 否则表示指定的图片已经添加水印
     */
    function watermark($source_image, $water = array(), $postion = 0, $border = 5, $lib = 0) {
        $border = is_numeric($border) ? $border : intval($border);
        if ($lib) {
