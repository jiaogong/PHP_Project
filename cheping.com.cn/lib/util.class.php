...
class util {
     ...
     * 生成带水印的图片
     * 
     * @param string $watermark 水印文件
     * @param string $source_image 要处理的图片
     * @param string $watermark_location 水印添加在原图上的位置
     * @param int $border 水印离图片边缘的距离
     * @param int $wm_w 水印宽度
     * @param int $wm_h 水印高度
     * @return resource 合成带水印的图片文件
     */
    public static function watermark($watermark, $source_image, $watermark_location = 2, $border = 5, $wm_w = 0, $wm_h = 0) {
        /* $watermark是水印图片全路径加文件名 */
        /* $source_image是要加水印的图片全路径加文件名 */
        /* $watermark_location是水印要加在图片的哪个位置(1为左下角,2为右下角,3为左上角,4为右上角,5为居中) */
        ...
        switch ($watermark_location) {
            ...
            case '6':
                $src_width = (int) ($source_image_width - $watermark_width) / 2;
                $src_height = $source_image_size[1] - $watermark_height;
                break;
            case '7':
                $src_width = (int) ($source_image_width - $watermark_width) / 2;
                $src_height = $border;
        ...
        if ($source_image_size[2] == 2) {
            ...
            @imagejpeg($nimage, $source_filename); //保存此图片,如无$target_image则输出到浏览器一个jpeg格式图片
            ...
            imagecopy($nimage, $source_image, 0, 0, 0, 0, $source_image_width, $source_image_height); //复制源图片到新建图片
            $merge_func($nimage, $watermark, $src_width, $src_height, 0, 0, $watermark_width, $watermark_height, $watermark_size[0], $watermark_size[1]); //复制水印图片到新建图片
            @imagepng($nimage, $source_filename); //保存此图片,如无$target_image则输出到浏览器一个jpeg格式图片
        } else {

            echo "<script>alert('不支持" . $source_image_size[2] . "图片格式')</script>";
            exit;
        }
