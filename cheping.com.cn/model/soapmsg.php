 ...
 * $Id: soapmsg.php 971 2015-10-22 04:09:19Z xiaodawei $
...
class soapmsg extends model {
    ...
    //24小时内请允许使用的短信条数限制
    private $sms_allow_num = array(
        'single_id' => 5,
        'single_ip' => 25,
    );
    private $test_ip = array(
        '61.149.73.65',
        '114.245.214.183',
        '114.245.219.173',
        '114.245.209.182',
    ...
    function __construct() {
        ...
        $this->client->soap_defencoding = 'GB2312';
     ...
     * @param bool   发送状态     2.成功  4.每天发送短信的次数超过5次不允许发送
     */
...
//        }
        ...
        $lastTimestamp = strtotime("-5 day");
        //当天该手机发送的信息总数
        $count = $this->shortMsg->getMsg('count(*)', "mobile = '$mobile' AND state = 2 AND sendtime > $timestamp", 3);
        //屏蔽连续5天发送超过25条短信
        $lastCount = $this->shortMsg->getMsg('count(*)', "state = 2 AND sendtime > $lastTimestamp AND ip = '$ip'", 3);
        if (in_array($ip, $this->test_ip) || strpos($ip, '192.168.') !== FALSE ||
                ($count < $this->sms_allow_num['single_id'] && $lastCount < $this->sms_allow_num['single_ip'])) {
            $flag = 0;
            $sn = $this->sn;
            ...
            $argv = array(
                ...
                'content' => iconv('utf-8', 'gbk', $content . $this->sms_keyword), //短信内容
                'ext' => '', //扩展码  可为空
    ...
    function callmsg($method, $parameters) {
        ...
        if (!$err = $this->client->getError() && $str === 'theone')
            return true;
        else
            return false;
    }
