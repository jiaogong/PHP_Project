    ...
    <div class="tppd_container">
        ...
        <div id="frame_left" class="frame_left">
            ...
            <DIV id="frame_tree" class="frame_tree">
                ...
                <div class="<? if ($brand_id==$bkey) { ?>jiajie1<? } else { ?>jiajie<? } ?>" bid="<?=$bkey?>">
        ...
        <div class="tkpp_right" style="padding-left:230px;">
            ...
            <? foreach((array)$list['factory'] as $key=>$value) {?>
            <!--<!-筛选项目1->-->
            <div class="tkpd_modle">
                <div class="tkpd_modle_title"><h3 style="font-size:18px; float:left;"><?=$value['factory_name']?></h3><span style=" font-size:12px;">（<? if ($value['count']) { ?><?=$value['count']?>张<? } else { ?>暂无<? } ?>)</span></div>
                <div class="tkpd_modle_content modle_content">
                    <? $i=0; ?>
                    <? foreach((array)$value['series'] as $k=>$v) {?>
                    <? $i++; ?>
                    <dl <? if ($i%4==0) { ?>style="margin:0px;"<? } ?>>
                        <dt>
                        ...
                        <dd><a href="image_SearchList_series_id_<?=$k?>.html" title="<?=$value['factory_name']?> <?=$v['series_name']?>" alt="<?=$v['series_name']?>"><? echo dstring::substring($v['series_name'],0,8) ?>(<? if ($v['count']) { ?><?=$v['count']?>张<? } else { ?>暂无<? } ?>)</a>
                            <p style="color:#de0312;"><? if ($v['created']) { ?><? echo date('Y-m-d',$v['created']) ?>更新<? } else { ?>暂无更新<? } ?></p>
                        </dd>
                    </dl>
                    <?}?>
                </div>
