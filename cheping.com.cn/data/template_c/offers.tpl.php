                    ...
                    <dd>
                        <div style="margin:0px;">
                            <div style="float:left;">
                                ...
                                </span><span style="font-size:14px; color:#000;">万</span><? } else { ?>未获取</span><? } ?>
                            </div>
                            ...
                            <? } ?>
                                        <? if ($model_info['privilege']) { ?><span class="sq_hui">惠 <em><?=$model_info['privilege']?></em></span><? } ?></div>
                                    <div class="clear"></div>
                            ...
                            <div class="sqhz_cont">
                                ...
                                <? foreach((array)$offers as $k=>$v) {?>
                                <a name="shangqing_<? echo $offers[$k+1]['id'] ?>"></a>
                                <div class="sqhz_cont_top1">
                                    <div class="i-tabs">
                                        <div class="i-tabs-nav">
                                            <span class="i-tabs-item <?=$v['item_active'][0]?>">详细内容 </span> 
                                            <? if ($loanArr&&$model_info['hasloan']) { ?><span class="i-tabs-item <?=$v['item_active'][11]?>">贷款方案 </span> <? } ?>
                                            <? if ($v[report_title1]) { ?>  <span class="i-tabs-item <?=$v['item_active'][1]?>"><?=$v[report_title1]?></span> <? } ?>
                                            <? if ($v[report_title2]) { ?>  <span class="i-tabs-item <?=$v['item_active'][2]?>"><?=$v[report_title2]?></span> <? } ?>
                                            <? if ($v[report_title3]) { ?>  <span class="i-tabs-item <?=$v['item_active'][3]?>"><?=$v[report_title3]?></span> <? } ?>
                                        ...
                                        <div class="i-tabs-container">
                                            <div class="i-tabs-content <?=$v['content_active'][0]?>">
                                                ...
                                                <div class="top3_container">
                                                    ...
                                                    <div class="top3_left">
                                                        <dl style="float:left">
                                                            <? $pics = explode('|', $v['dealer_pic']) ?>
                                                            <? $pic_alt = explode('|', $v['dealer_pic_alt']) ?>
                                                            <dt style="position:relative;"><img width="210" <? if ($pic_alt[0]) { ?>alt='<?=$pic_alt[0]?>'<? } ?> height="158" src="<? if ($pics[0]) { ?><?=$pics[0]?><? } else { ?>11<? } ?>" onerror="this.src='images/offerrs_dealer.jpg'" /></dt>
                                                        </dl>
                                                        <div class="top3_right_right">
                                                            <dl>
                                                                <dd style="font-size:14px;color:#000;" <? $add_1 = strlen($v['dealer_name']) ?><? if ($add_1>40) { ?>title="<?=$v['dealer_name']?>"<? } ?>><? echo dstring::substring($v['dealer_name'],0,42) ?> </dd>
                                                                <dt><img src="images/sq_4s.jpg"></dt>
                                                            </dl>

                                                            <p style="color:#585858;" <? $add = strlen($v['dealer_addr']) ?><? if ($add>42) { ?>title="<?=$v['dealer_addr']?>"<? } ?>>地址：<? echo dstring::substring($v['dealer_addr'],0,42) ?></p>
                                                        </div>
                                                        <div class="clear"></div>
                                                    </div>
                                                    <div class="top3_right">
                                                        <div>
                                                            ...
                                                            <p style="height:38px; line-height:38px; clear:both;"><span style="font-size:32px; color:#ed7001;"><?=$v[price]?></span><span style="font-size:14px; color:#000;">万</span><? if ($v['privilege_hui']) { ?><span class="sq_hui2">惠 <em><?=$v['privilege_hui']?></em></span><? } ?></p>
                                                        </div>
                                                        ...
                                                        <div class="bg_left">获取方式：<? if ($v['price_type'] == 0) { ?>
                                                            ...
                                                            汽车之家<? } ?></div>
                                                        <div class="bg_right">获取/核实时间：<? if ($v['get_time']) { ?><? echo date("Y-m-d",$v['get_time']) ?><? } ?><br>
                        ...
                        <div class="content_right">
                            ...
                            <div class="content_right_modle">
                                ...
                                <? $ii=count($compete_model) ?>
                                <? foreach((array)$compete_model as $k=>$v) {?>
                                <div class="content_right_container">
                                    ...
                                    <div class="cont_right">
                                        <div style="padding:0px 0px 0px 4px;">
                                            <div style="color:#000;">
                                                <div style="float:left;">网络媒体价： 
                                                    <span style="font-size:15px; color:#ff7e0d;">
                                                        <? if ($v[price]>0) { ?><?=$v[price]?><? } elseif($v[model_price]) { ?><?=$v[model_price]?><span style="font-size:12px">万</span><? } else { ?>未获取<? } ?>
                                                    </span>
                                                </div>
                                                <? if ($v['price_type'] == '5') { ?>
                                            ...
                                            <p style="margin-top:8px;">获取渠道：<? if ($v['price_type'] == '5') { ?>汽车之家<? } else { ?>无<? } ?></p>
                                            <p>获取方式：<? if ($v['price_type'] == '5') { ?>网络媒体<? } else { ?>无<? } ?></p>
                                            <p class="ckxq"><a href="offers_<?=$v[model_id]?>.html" onfocus="this.blur()" target="_blank">查看详情</a></p>
                                        </div>
                                    </div>
                                </div>
                                <? if ($k+1!=$ii) { ?><div class="ckyx_bor"></div><? } ?>
                                <?}?> 
                            </div>
                            <? } ?>
                            <!--#include virtual="ssi/ssi_offers_price.shtml"-->
                            <? if (!empty($article)) { ?>
                            <div class="content_right_modle"<? if (!empty($compete_model)) { ?> style="margin-top:10px;"<? } ?>}>
                                ...
                                <div class="cxxx_module" style="padding-top:5px;">
                                    <div class="cxxx_title"><a href="html/article/<? echo date('Ym/d', $art['created']) ?>/<?=$art['id']?>.html" title="<?=$art['full_title']?>" target="_blank"><?=$art['short_title']?></a></div>
                                    <div class="cxxx_date"><? echo date("Y-m", $art['created']) ?></div>
                                </div>			
                                <? } ?>
                            </div>        
                            <? } ?>
