...
<!--头部结束-->
         <div class="userorder">
              <div class="order-top">
                  <div class="fl"><img src="<?php echo ($pic_path); ?>" width="520px" height="390px" /></div> 
                  <div class="orderdis fl">
                       <span>作品名称：<?php echo ($model_data["title"]); ?></span>
                       <span>作品分类：<?php echo ($model_data["opus_type"]); ?></span> 
                       <span>品牌产品：<?php echo ($model_data["car_logo"]); ?> <?php echo ($model_data["car_type"]); ?> <?php echo ($model_data["car_model"]); ?></span>
                       <span>标签：<?php echo ($model_data["tags"]); ?></span> 
                       <span>创建时间：<?php echo ($model_data["select_time"]); ?></span>
                       <span>模型面数：<?php echo ($model_data["model_faces"]); ?>面</span> 
                       <span>模型格式：<?php echo ($model_data["model_format"]); ?></span>
                       <span>模型精度：<?php echo ($model_data["precision"]); ?></span>         
                  </div>
                  <div class="clear"></div>
              </div> 
                   ...
                   <input type="hidden" name="price" value="" />
                       <ul>
                          <li> 
                             ...
                             <label  class="checked" use='price_a' price="<?php echo ($model_data["price_a"]); ?>" >使用许可：6个月</label>
                            
                            <span class="fr">￥<?php echo ($model_data["price_a"]); ?>元</span>
                          </li>
                        
                            <li> 
                             <input type="radio"  name="sport"  >
                             <label use='price_b' price="<?php echo ($model_data["price_b"]); ?>">使用许可：12个月</label>
                             <span class="fr">￥<?php echo ($model_data["price_b"]); ?>元</span>
                          </li>
                           <li> 
                             <input type="radio"  checked="checked" name="sport"  >
                             <label  use='price_c' price="<?php echo ($model_data["price_c"]); ?>">使用许可：24个月</label>
                             <span class="fr">￥<?php echo ($model_data["price_c"]); ?>元</span>
                          </li>
                           <li> 
                             <input type="radio"   name="sport"  >
                             <label use='price_d' price="<?php echo ($model_data["price_d"]); ?>" >著作权转让</label>
                             <span class="fr">￥<?php echo ($model_data["price_d"]); ?>元</span>
                          </li>
