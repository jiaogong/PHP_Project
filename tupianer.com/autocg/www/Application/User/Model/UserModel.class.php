...
//        array('mobile', '', -11, self::EXISTS_VALIDATE, 'unique'), //手机号被占用
     ...
     */
    public function register($username, $password, $email, $mobile, $gender, $group_id) {
        switch ($group_id) {
            case 4:
                $group_id = 1;
                break;
...
//    }
    ...
    public function info($uid, $is_username = false, $email = false) {
        ...
        if (is_array($user)) {
            return array($user['uid'], $user['username'], $user['email'], $user['phone']);
        } else {
            return -1; //用户不存在或被禁用
        }
    }
