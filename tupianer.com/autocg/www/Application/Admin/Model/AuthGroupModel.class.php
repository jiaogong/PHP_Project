...
class AuthGroupModel extends Model {
     ...
     */
    public function getQgroups($where = array()) {
        $map = array('status' => 1, 'type' => self::TYPE_ADMIN, 'module' => 'home');
        $map = array_merge($map, $where);
        return $this->where($map)->select();
    }
     ...
     * 示例: 把uid=1的用户添加到group_id为1,2的组 `AuthGroupModel->addToGroup(1,'1,2');`
     */
    public function addToGroups($uid, $gid) {
        $uid = is_array($uid) ? implode(',', $uid) : trim($uid, ',');
     ...
     * @author 崔元欣 <15811506097@163.com>
     */
    public function checkIds($modelname, $mid, $msg = '以下id不存在:') {
        if ($mid) {
            $ids = $mid;
        } else {
            $this->error = $msg . $mid;
    ...
    public function checkGroupId($gid) {
        if (is_array($gid)) {
            ...
            return $this->checkIds('AuthGroup', $gid, '以下用户组id不存在:');
        }
    }
