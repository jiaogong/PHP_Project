 ...
 */
class AuthFrontController extends AdminController {
    ...
    public function index() {
        $list = $this->lists('AuthGroup', array('module' => 'home'), 'id asc');
    ...
    public function editGroup() {
        $auth_group = M('AuthGroup')->where(array('module' => 'home', 'type' => AuthGroupModel::TYPE_ADMIN))
                ->find((int) $_GET['id']);
...
//        }
//
//        $auth_group = M('AuthGroup')->where(array('status' => array('egt', '0'), 'module' => 'home', 'type' => AuthGroupModel::TYPE_ADMIN))
//                ->getfield('id,id,title,rules');
//        $prefix = C('DB_PREFIX');
//        $l_table = $prefix . (AuthGroupModel::USER);
...
//    }
     ...
     * @author 崔元欣 <15811506097@163.com>
     */
    public function updateRules($condition = 'home') {
        //需要新增的节点必然位于$nodes
        $nodes = $this->returnNodes(false,$condition);

        $AuthRule = M('AuthRule');
        $map = array('module' => $condition, 'type' => array('in', '1,2')); //status全部取出,以进行更新
     ...
     * @author 崔元欣 <15811506097@163.com>
     */
    public function access($condition = 'home') {
        $this->updateRules();
        $auth_group = M('AuthGroup')->where(array('status' => array('egt', '0'), 'module' => $condition, 'type' => AuthGroupModel::TYPE_ADMIN))
                ->getfield('id,id,title,rules');
        $node_list = $this->returnNodes(true,$condition);
        $map = array('module' => 'home', 'type' => AuthRuleModel::RULE_MAIN, 'status' => 1);
        $main_rules = M('AuthRule')->where($map)->getField('name,id');
        $map = array('module' => 'home', 'type' => AuthRuleModel::RULE_URL, 'status' => 1);
