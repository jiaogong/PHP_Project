...
class AdminController extends Controller {
     ...
     * @author 朱亚杰 <xcoolcc@gmail.com>
     */
    final protected function returnNodes($tree = true,$condition = 'admin') {
        static $tree_nodes = array();
        if ($tree && !empty($tree_nodes[(int) $tree])) {
            return $tree_nodes[$tree];
        }
        if ((int) $tree) {
            $list = M('Menu')->field('id,pid,title,url,tip,hide')->where(array('condition ' => $condition))->order('sort asc')->select();
            foreach ($list as $key => $value) {
                if (stripos($value['url'], MODULE_NAME) !== 0) {
                    if($condition == 'admin'){
                        $list[$key]['url'] = MODULE_NAME . '/' . $value['url'];
                    }else{
                        $list[$key]['url'] = 'Home/' . $value['url'];
                    }
                }
            }
            ...
            $nodes = M('Menu')->field('title,url,tip,pid')->where(array('condition ' => $condition))->order('sort asc')->select();
            foreach ($nodes as $key => $value) {
                if (stripos($value['url'], MODULE_NAME) !== 0) {
                    if($condition == 'admin'){
                        $nodes[$key]['url'] = MODULE_NAME . '/' . $value['url'];
                    }else{
                        $nodes[$key]['url'] = 'Home/' . $value['url'];
                    }
                }
            }
        }
